...
//		Map은 key의 자료형과 value의 자료형을 모두 지정해야 한다
		...
		Set<String> keySet = map.keySet();			// key는 중복될 수 없어서 Set 으로 받으면 된다
...
//		순번이 필요하다면 LinkedHashMap으 사용할 수 있고
//		key와 value의 제네릭 타입이 모두 String이면서 파일에 저장해서 불러올경우는
//		Properties 타입을 사용할 수 있다
		
		Properties pro = new Properties();
		pro.put("charAt", "인덱스를 전달하여 글자를 char형태로 반환한다");
		pro.put("equals", "다른 문자열과 비교하여 내용 일치여부를 boolean으로 반환");
		pro.put("contains", "특정 패턴의 내용잉 포함되는지를 boolean으로 반환");
		pro.put("substring", "인덱스를 지정하여 특정 범위의 문자열을 잘나낸다");
		pro.put("split", "특정 글자를 기준으로 각 내용을 분리하여 Srring[]로 반환한다");
...
//		클래스를 작성하는대신, Map에 저장하여 한번에 전달하는 용도로도 사용한다
