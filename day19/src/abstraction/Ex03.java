...
abstract class Test3 {
	void function1() {					// 추상 클래스. 객체화 할 수 없다.
		System.out.println("내용");		// 일반 메서드를 포함할 수 있다 (추상메서드도 포함할 수 있다)
...
//	1) Sub를 추상클래스로 만들면 추상메서드를 포함할수 있다
//	★	2) 추상메서드를 오버라이딩해서, body를 만들어 주면 더이상 추상메서드가 아니게 된다(일반메서드가 된다)
	
//	The type Sub must implement the inherited abstract method Test3.function2()
//	자료형 Sub는 상속받은 Test3의 추상메서드 function2를 반드시 구현해야 한다(추상메서드는 미구현으로 간주)
...
//		추상 메서드 : 함수의 형태는 지정되어 있으나, 내용은없어서 호출할 수 없는 함수
