...
public class Ex01 {
	...
	public static void main(String[] args) {
		...
		// 제어문
		// control statement
		// 코드의 흐흠 및 상태를 제어하는 구문
		
		// 1) 분기문 : 조건에 따라 서로 다른 코드를 실행한다 (if, switch ~ case)
		
		// 2) 반복문 : 조건에 따라 특정 커드를 반복수행한다 (while, for)
		
		// 3) 기타 : 반복흐름 강제로 끊거나, 함수진행을 강제로 종료한다 (break, continue, return)
		
		
		// 분기문과 반복문은 특정 조건에 의해서 수행되므로
		// 비교연산, 논리연산 등을 활영하여 boolean 타입의 값을 다루는 연습이 필요하다
		
		
		// >, <, >=, <=, ==, !=			일반적으로 수치에 대한 비교를 수행하여 boolean을 만들어낸다
		// &&, || , !					boolean을 이용하여 새로운 boolean을 만들어낸다 
		
		// 사용자의 "입력값" 무엇이냐에 따라, 어떤 크기의 값이냐에 따라
		// 조건을 생성하고, 조건에 따라 서로 다른 코드를 수행하게 한다
		// 분기분 뿐만 아니라, 반복문도 boolean 타입의 값을 사용하기 때문에
		// 한글로 번역했을 때 "만약에" 라는 형식의 구문이 들장하는 경우가 많다
		...
		if (isOdd) {								// 조건에 따라 코그 수행 (입력값이 홀수이면)
