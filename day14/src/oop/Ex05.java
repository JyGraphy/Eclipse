...
class PhoneBook {						// 내가 만드는 자료형
	...
	// 생성자 : 객체를 생성할 때 실행되는 함수이며, 클래스의 이름과 같은 이름을 사용하는 함수 / 초기 작업을 진행하는 함수
	// 생성자 정의에는 반환형 자체를 작성하지 않는다
	PhoneBook(String name, int age, String phoneNumber) {
		this.name = name;			// 매객변수에 넘어온 값을 같은 이름의 멤버필드에 저장한다
		this.age = age;
...
public class Ex05 {
	...
	public static void main(String[] args) {
		...
		// 클래스를 이용하여 객체를 생성할 때는 [생성자]라고 하는 특수한 함수를 호출
		// 자바에서는 몇가지 예외상황을 제외하면, 생성자를 통해서만 객체를 생성할 수 있다
		// 아래 코드에서 ob1은 객체가 생성되었지만
		// ob2는 객체를 가리킬 수 있는 참조 변수만 생성되었을뿐, 실제 객체는 생성되지 않은 상태이다
		...
		// 객체 내부의 맴버필드에도 일반 변수와 동일하게 대입을 수행할 수 있다
		
		ob1.name = "이지은";
		ob1.age = 30;
		ob1.phoneNumber = "010-1234-1234";
		
		// 필드에 접근하여 값을 출려할 수도 있지만, 만들어둔 기능이 있으면 함수를 호출할 수 있다
		ob1.show();
		
		// 아까 문들어둔 ob2 변수에도 새로운 객체를 할당하여 추가 데이터를 입력한다
		ob2 = new PhoneBook();
