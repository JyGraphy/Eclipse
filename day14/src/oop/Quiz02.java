...
class Customer {
	// 1) 문자열 형태의 이름과 정수 형태의 나이를 저장할 수 있는 필드를 작성하시오
	String name;
	int age;
	
	// 2) 손님 객체는 반드시 이름과 나이가 지정되어 있어야 합니다
	// 생성자를 활영하여 매개변수로 이름과 나이를 전달받은 후 필드에 저장하도록 하세요
	...
	// 7) 생성자를 추가로 작성하여(오버로딩), 이름만으로도 갹체를 생성할 수 있도록 한다
	// 이때, 값을 초기화 하지않은 age는 0이다
...
public class Quiz02 {
	...
	public static void main(String[] args) {
		...
		Customer n1 = new Customer("이선균", 41);
		Customer n2 = new Customer("권지용", 36);
		Customer n3 = new Customer("전청조", 15);
		Customer n4 = new Customer("남현희");
		
		// 6) 손님 객체 3명을 배열로 묶은 다음
		// 반복문을 이용하여 손님의 성인 여부에 따라 판매 가능/ 불가능을 출력합니다.
		Customer[]arr = {n1, n2, n3, n4};
		for (int i = 0; i < arr.length; i++) {
			Customer member = arr[i];					// 배열 안의 각 손님 객체를 member라고한다
			member.show();								// 손님 정보를 출력
			if (member.isAdult()) {						// 손님이 성인이면
				System.out.println("상품 판매 가능");		// 판매가능
			}
			else {										// 성인이 아니면 
				System.out.println("상품 판매 불가능");		// 판매 불가능
			}
			System.out.println();						// 구분을 위한  한줄 바꾸기
		}
