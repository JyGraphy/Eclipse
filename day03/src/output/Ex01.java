...
public class Ex01 {
	...
	public static void main(String[] args) {
		...
		//System.out.print(); : 괄호안에 하나의 값을 받아서 출력한다.
							// 줄을 바꾸지 않는다
		...
		System.out.print("\n"); // \n른 줄바꿈 문자(개행문자, newLine, LineFeed)이다
		System.out.print("B"); // 따라서 print("\n")과 println()은 같은 역할이다
		System.out.print("\r"); // \r은 커서를 줄 왼쪽끝으로 이동시킨다 (CarrigeReturn)
		System.out.print("C"); // \r도 엔터의 값이므로 줄을 바꾼다
		System.out.println(); // 보통 줄만 바꾸기 싶으면 println()을 사용한다
		
		String lyString = "동해물과 백두산이 마르고 닳도록\r\n";
		lyString += "하느님이 보우하사 우리나라 만세\r\n";
		
		// + 연산자는 숫자간의 연산에서 숫자를 계산하고
		// 문자열이 양쪽중 한곳에 포함되면, 문자열을 베이스로 하여 굴자를 이어붙이기 한다(append)
		// 산술연산자는 복합대입연산자 형식으로 사용할 수 있기 때문에
		// lyrics에 처름 값을 지정한 후
		// 기존 값을 유지하면서 새로운 내용를 덧붙이기(append)할때 사용할 수 있다
		
		
		System.out.print(lyString);
		...
		for (int i = 0; i < 5; i++) {
			System.out.println(i);	// 값을 출력할떄 마다 계속 줄 바꾸기
		}
